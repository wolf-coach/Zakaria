  ...
  useEffect(() => {
    ...
    const unsubscribeProfile = onValue(profileRef, (snap) => {
      ...
      notify(e?.code === "PERMISSION_DENIED"
        ? "Firebase denied your profile. Check Realtime Database Rules."
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      try {
        ...
        notify("Signed in, but your profile could not be loaded. Check Realtime Database Rules.");
      }
    });
  ...
  async function handleAuth(mode, form) {
    try {
      ...
      const msg = e?.code === "auth/invalid-credential" ? "Incorrect email or password."
        ...
        : e?.code === "PERMISSION_DENIED" || e?.code === "permission-denied" ? "Firebase blocked this request. Check Realtime Database Rules."
